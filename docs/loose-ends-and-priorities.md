...
| # | Item | State | Evidence / Notes |
|---|---|---|---|
| 2.3.1 | Add `earthFrame` SCNNode at scene-graph root | 🟡 Spec written (v2) | Design spec: `docs/superpowers/specs/2026-06-30-earthframe-node-hierarchy-design.md` (v2, scrutinized 2026-07-01). Pending implementation. |
| 2.3.2 | Move anchors/content under `earthFrame/anchors/<id>` | 🟡 Spec written (v2) | Same spec. Scope expanded: self-cleaning removal across **all** reset/restart/failure paths (`viewWillAppear`, `startGeoTrackingSession`, LIDAR, high-accuracy reload, `session:didFailWithError:`) — not just the reload — after scrutiny found v1's defense-in-depth was incomplete. `unloadModel` made `didRemove`-authoritative. Per-branch reparent, idempotent creation, `didSet`+pure-validator invariant, race fix. |
